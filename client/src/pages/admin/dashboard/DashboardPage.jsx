...
export const Dashboard = (props) => {
  ...
  return (
    <Container>
        { data && data.role && data.role.filter(value => value.authority==='USER').length>0 ? (
            <MainWrapper>
                <h4>Hello {data && `${data.firstName} ${data.lastName}`}</h4>
                <br></br>
                {data && data.role && data.role.filter(value => value.authority==='USER').length>0 && <Button type="variant">Add User</Button> }
                <br></br>
