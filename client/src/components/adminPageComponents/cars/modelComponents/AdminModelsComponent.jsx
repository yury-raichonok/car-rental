...
const AdminCarsComponent = (props) => {
  ...
  function EditModelComponentFunction(props) {
    return <EditModelComponent model={props.model} handleEditInfoCancel={handleEditInfoCancel} fetchModels={fetchModels} />
  }

  const fetchModels = async () => {
    setLoading(true);

    const resp = await ModelDataService.getModelsListWithBrandName(state.pageNumber, state.pageSize, state.sortBy, state.sortDirection).catch((err) => {
      notification.error({
        message: `${t('error_when_fetching_models_list')}`,
      });
      setLoading(false);
    });
  ...
  return (
    <div>
      <FunctionsContainer>
        <ButtonsContainer>
          <button className="btn" onClick={showModelModal}>
            {t('add_new_model')}
          </button>
      ...
      <Modal
        ...
        title={t('add_new_car_model')}
        onCancel={handleModelCancel}
        footer={[]}
      >
        <AddModelComponent handleModelCancel={handleModelCancel} fetchModels={fetchModels} />
      </Modal>
      <Modal
        width="600px"
        visible={showEditModal}
        title={t('edit_car_model')}
        onCancel={handleEditInfoCancel}
        footer={[]}
      >
        <EditModelComponentFunction model={model} />
      </Modal>
    </div>  
  )
}
