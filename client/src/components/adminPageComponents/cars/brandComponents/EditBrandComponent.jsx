...
const EditBrandComponent = (props) => {
  ...
  const onFinish = async (values) => {
    ...
    BrandDataService.update(props.data.id, values).then(
      res => {
        if(res.status === 200) {
          onReset();
          props.handleEditBrandCancel();
          props.fetchBrands();
          notification.success({
            message: `${t('car_brand_updated')}`,
          });
        }
      }
    ).catch(
      err => {
        if(err && err.response){
          switch(err.response.status){
            ...
            default:
              notification.error({
                message: `${t('error_when_updating_brand')}`,
              });
          }
        }
      }
