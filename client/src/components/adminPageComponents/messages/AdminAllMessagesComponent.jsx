...
const AdminAllMessagesComponent = () => {
  ...
  const fetchMessages = async () => {
    const resp = await MessageDataService.getAllMessages(state.pageNumber, state.pageSize, state.sortBy, state.sortDirection).catch((err) => {
      notification.error({
        message: `${t('error_when_fetching_messages_list')}`,
      });
      setLoading(false);
    });
  ...
  const AllMessagesTableColumns = [
    ...
    { 
      ...
      dataIndex: 'readed',
      key: 'readed',
      sorter: (a, b) => a.readed - b.readed,
      render: (dataIndex) => <div>{dataIndex ? `${t('readed')}` : `${t('new')}`}</div>
    }
  ];
