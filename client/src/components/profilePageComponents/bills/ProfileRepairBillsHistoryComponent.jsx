...
const ProfileRepairBillsHistoryComponent = () => {
  ...
  const fetchUserRepairBillsHistory = async () => {
    setLoading(true);

    const resp = await RepairBillDataService.findUserBills(state.pageNumber, state.pageSize, state.sortBy, state.sortDirection).catch((err) => {
      notification.error({
...
export default ProfileRepairBillsHistoryComponent
